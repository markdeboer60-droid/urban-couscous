...
export function buildGoamlXml(p: GoamlMeldingParams): string {
  ...
  const lines: string[] = [
    ...
    `  <rentity_id>${esc(p.kantoorKvk ?? p.meldingId)}</rentity_id>`,
    `  <rentity_branch>${esc(p.kantoorNaam)}</rentity_branch>`,
    ...
    `  <transaction>`,
    `    <transactionnumber>${esc(p.meldingId)}</transactionnumber>`,
    ...
    `    <transaction_description>${esc(p.omschrijving)}</transaction_description>`,
    `    <reason>${esc(p.motivatie)}</reason>`,
    `    <indicator_type>${indicatorCode}</indicator_type>`,
    `    <additional_info>${esc(p.onderbouwing)}</additional_info>`,
    `    <t_from_my_client>`,
    `      <from_funds_code>U</from_funds_code>`,
    `      <from_country>NL</from_country>`,
    `      <entity>`,
    `        <name>${esc(p.clientNaam)}</name>`,
    p.clientKvk ? `        <registration_number>${esc(p.clientKvk)}</registration_number>` : "",
    ...
    `  </transaction>`,
    p.fiuReferentie
      ? `  <fiu_ref_number>${esc(p.fiuReferentie)}</fiu_ref_number>`
      : "",
    `</report>`,
  ];
